  ...
  <main id="main-area" class="main-stage modern-main-stage">
    <header id="header" class="main-header modern-main-header">
      ...
      <div class="main-header-actions modern-main-actions">
        ...
        <span class="main-header-pill theme-pill">{{ activeThemeName }}</span>
      </div>
    </header>
    ...
    <button
      v-if="hasMediaSurface"
      type="button"
      class="stage-status-toggle"
      :class="{ open: dashboardOpen }"
      @click="toggleDashboard"
    >
      <span>{{ dashboardOpen ? '收起状态' : '状态面板' }}</span>
      <strong>{{ connectionStats.qualityLabel }}</strong>
    </button>

    <section
      class="stage-dashboard stage-dashboard-overlay"
      :class="{
        connected: isConnected,
        'media-active': hasMediaSurface,
        visible: dashboardOpen,
        collapsed: !dashboardOpen,
      }"
    >
      <div class="stage-hero-card glass-panel">
        <div class="stage-hero-copy">
          <div class="stage-kicker">DoNiChannel Voice Console</div>
          <h2>{{ currentChannelName }}</h2>
          <p>{{ memberCount }} 人在频道内 · {{ connectionSummary }}</p>
        </div>

        <div class="stage-self-card">
          <BaseAvatar
            :name="selfName"
          ...
          <div>
            ...
            <span>{{ profileStore.statusText || '在线' }}</span>
          </div>
        </div>
      </div>

      <div class="stage-grid">
        <section class="stage-panel stage-members-panel glass-panel">
          ...
          <div v-if="visibleMemberCards.length > 0" class="stage-member-grid">
            <article
              v-for="member in visibleMemberCards"
              :key="getMemberIdentity(member) || getMemberName(member)"
              class="stage-member-card"
              :class="{ self: isSelfMember(member) }"
            >
              <BaseAvatar
                ...
                online
              />
              <div class="stage-member-copy">
                <strong :title="getMemberName(member)">{{ getMemberName(member) }}</strong>
                <span>{{ isSelfMember(member) ? '你正在此频道' : '语音成员' }}</span>
              </div>
              <span class="stage-member-mic">🎙️</span>
            </article>
          </div>

          <div v-else class="stage-empty-state compact">
            <strong>还没有成员进入语音频道</strong>
            <span>选择左侧频道后，这里会显示当前频道成员。</span>
          </div>
        </section>

        <section class="stage-panel stage-media-panel glass-panel">
          <div class="stage-panel-header">
            <div>
              <strong>媒体状态</strong>
              <span>本机发布状态</span>
            </div>
            <em>{{ appStore.media.micSource === 'rust' ? 'Rust Mic' : 'Browser Mic' }}</em>
          </div>

          <div class="stage-media-grid">
            <div
              v-for="card in mediaStatusCards"
              :key="card.key"
              class="stage-media-card"
        ...
        <section class="stage-panel stage-quality-panel glass-panel">
          <div class="stage-panel-header">
            <div>
              <strong>连接质量</strong>
              <span>本地 WebRTC 统计，不请求服务器</span>
            </div>
            <em>Stage 26</em>
          </div>

          <div class="quality-metrics-grid">
            <div class="quality-metric-card" :class="connectionStats.qualityTone">
              <span>延迟</span>
              <strong>{{ latencyText }}</strong>
              <em>Round Trip Time</em>
            </div>
            <div class="quality-metric-card" :class="connectionStats.qualityTone">
              <span>丢包</span>
              <strong>{{ packetLossText }}</strong>
              <em>Inbound RTP</em>
            </div>
            <div class="quality-metric-card">
              <span>码率</span>
              <strong>{{ bitrateText }}</strong>
              <em>本地 getStats</em>
            </div>
          </div>

          <div class="quality-sparkline" aria-hidden="true">
            <span
              v-for="(sample, index) in connectionStats.samples"
              :key="`${sample}-${index}`"
              :style="{ height: `${sample}%` }"
            ></span>
          </div>
